...
export class InputSelectOptionField implements OnInit, OnDestroy {
  ...
  private _syncDisplayFromValue(val: any) {
    const opts = this.options() || [];
    // Try by key first (strict), then by stringified key, then by display value
    let selected = opts.find(o => o.key === val);
    if (!selected) {
      selected = opts.find(o => String(o.key) === String(val));
    }
    if (!selected && typeof val === 'string') {
      selected = opts.find(o => o.value === val);
    }
    if (selected) {
